...
public class UpmsUserAdapter extends AbstractUserAdapter {
    ...
    private volatile Set<RoleModel> cachedLoginUserRealmRoles;
	...
	public UpmsUserAdapter(KeycloakSession session, RealmModel realm, ComponentModel model, UpmsUser user, UpmsUserRepository repository) {
		...
		this.model = model;
		this.user = user;
		this.kcId = StorageId.keycloakId(model, user.getUserId());
	...
	protected Set<RoleModel> getRoleMappingsInternal() {
		if(cachedLoginUserRealmRoles == null) {
			String userId = StorageId.externalId(kcId);
	        List<UpmsRole> userRoles = getRepository().getUserRoles(userId);
	        LOGGER.info(String.format(">>> getUserRoles(%s) = %s", userId, userRoles));
	        if(userRoles != null) {
	        	cachedLoginUserRealmRoles = userRoles.stream().map(role -> new UpmsRoleAdapter(session, realm, model, role)).collect(Collectors.toSet());
	        } else {
	        	cachedLoginUserRealmRoles = Collections.EMPTY_SET;
	        }
		}
        return cachedLoginUserRealmRoles;
    }
