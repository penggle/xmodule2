package com.penglecode.xmodule.webflux.examples.service;
...
import com.penglecode.xmodule.webflux.examples.model.Joke;
import com.penglecode.xmodule.webflux.examples.support.OpenApiResult;
...
public class JokeService {
	...
	public Flux<Joke> getJokeList(String type, int page, int count) {
		...
		return webClient.get().uri("/getJoke?page={page}&count={count}&type={type}", parameter).retrieve()
				.bodyToMono(new ParameterizedTypeReference<OpenApiResult<List<Joke>>>() {
				}).flatMapMany(result -> {
	...
	public Mono<Joke> getJokeBySid(String sid) {
		return webClient.get().uri("/getSingleJoke?sid={sid}", Collections.singletonMap("sid", sid)).retrieve()
				.bodyToMono(new ParameterizedTypeReference<OpenApiResult<Joke>>() {
